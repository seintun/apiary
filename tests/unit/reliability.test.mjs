...
test('complete refuses unfinished scouts unless forced', () => {
  ...
  spawnSync(process.execPath, ['scripts/apiary-run.mjs', 'worker-start', '--run', runId, '--id', 'unfinished', '--label', 'Unfinished', '--role', 'test'], { cwd: repo })
  const blocked = spawnSync(process.execPath, ['scripts/apiary-run.mjs', 'complete', '--run', runId], { cwd: repo, encoding: 'utf8' })
  assert.notEqual(blocked.status, 0)
  assert.match(blocked.stderr, /unfinished workers/)
