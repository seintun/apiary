...
test('no config omits explicit model', () => {
  const resolved = resolveApiaryModel({}, 'cheapScout')
  assert.equal(resolved.role, 'cheapScout')
  assert.equal(resolved.model, undefined)
  assert.equal(resolved.omitModel, true)
})

test('soft preferred model is applied', () => {
  const config = { apiary: { models: { cheapScout: { prefer: 'local-small' } } } }
  const { options, resolved } = buildSpawnOptions({ task: 'inspect files' }, config, 'cheapScout')
...
test('soft preferred model failure retries once with host default', async () => {
  const calls = []
  const config = { apiary: { models: { cheapScout: { prefer: 'flaky-model' } } } }
  const result = await spawnWithApiaryModel(async (options, resolved) => {
    ...
    return 'ok'
  }, { task: 'compare docs' }, config, 'cheapScout')
...
test('air-gapped default policy never requires an explicit model', () => {
  for (const role of ['cheapScout', 'balancedScout', 'strongJudge', 'reviewer', 'fallbackSafe']) {
...
test('ordered preferences choose first explicit model but still fallback softly', () => {
  const config = { apiary: { models: { cheapScout: { prefer: ['auto', 'local-small', 'cloud-small'] } } } }
...
test('all default-like ordered preferences omit model for host/local runtime', () => {
  const config = { apiary: { models: { cheapScout: { prefer: ['auto', 'default', 'host-default'] } } } }
  assert.equal(resolveApiaryModel(config, 'cheapScout').model, undefined)
})
